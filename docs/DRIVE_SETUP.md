...
> ⚠️ **フォルダの置き場所が重要です**
>
> サービスアカウント (SA) は自分のストレージ容量を持ちません（容量 0）。そのため：
> - **マイドライブ内のフォルダを共有した場合** … 添付ファイルのアップロードやフォルダ作成はできますが、**Google ドキュメントの新規作成（ドキュメント機能・AIドキュメント生成）が `storageQuotaExceeded` エラーで失敗します**
> - **共有ドライブ（Shared Drive）内に作成した場合** … すべての機能が動作します
>
> ドキュメント機能・AIドキュメント生成まで使う場合は、**共有ドライブ内にフォルダを作成してください**（共有ドライブの利用には Google Workspace が必要です。個人の無料 Gmail アカウントでは作成できません）。
> 添付ファイル機能だけで良い場合は、マイドライブ内のフォルダでも動作します。

Drive（推奨: 共有ドライブ）上で以下の3フォルダを作成：
...
### ドキュメントの新規作成だけ失敗する（`storageQuotaExceeded` / `Service Accounts do not have storage quota`）

→ フォルダが **マイドライブ内** にあることが原因。SA は自分の容量を持たないため、マイドライブ共有では新規 Google ドキュメントを作成できません（フォルダ作成や添付アップロードは通る場合があり、気づきにくい）。**共有ドライブ内のフォルダ**（要 Google Workspace）に置き換えてください。
